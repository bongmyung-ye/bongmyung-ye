<p align="center">
  <a href="https://crawlblog.xyz">
    <img src="https://cdn.discordapp.com/attachments/1418997178294079530/1523094785966211262/crawl11.png?ex=6a4adc0e&is=6a498a8e&hm=7ba322bdf20423c1cc07814908ef9d8987e16217a3664095dce3460e0594b785&" alt="Crawl Blog" />
  </a>
</p>

<div>
  <img align="right" width="400" alt="Shimarin" src="https://cdn.discordapp.com/attachments/1418997178294079530/1523097396081459270/rounded-in-photoretrica.png?ex=6a4ade7d&is=6a498cfd&hm=887a600727c4cf80a6240e611fe788b9a2c87e48b1cd6b8f242d7193291c4e78&" />

  <h2> / ABOUT ME / </h2>

- 커뮤니티 운영에 필요한 **Discord 봇, 자동화 기능, 웹 도구**를 개발
- **TypeScript · Node.js** 기반으로 명령어 처리, 상태 관리, 서버 연동 기능을 구현
- **Next.js · React**로 프로젝트 페이지와 웹 서비스를 제작하고 배포
- **Ubuntu VPS · PM2 · DNS** 환경에서 서비스 배포와 운영 경험
- 로그, 상태 확인, 통계, 활동 추적처럼 실제 운영에 필요한 기능을 자주 다룸
- **바이너리 분석, YARA 룰, PE 파일 분석**을 안전한 자체 샘플 기반으로 정리

<h2> / CURRENT SKILLS / </h2>

- <h4> core languages </h4>

  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white" alt="typescript" />
  <img src="https://img.shields.io/badge/JavaScript-323330?style=for-the-badge&logo=javascript&logoColor=F7DF1E" alt="javascript" />
  <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" alt="c" />
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="python" />
  <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="html5" />
  <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="css3" />

- <h4> backend & bot development </h4>

  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white" alt="nodejs" />
  <img src="https://img.shields.io/badge/Discord.js-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="discordjs" />
  <img src="https://img.shields.io/badge/Sapphire_Framework-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="sapphire framework" />
  <img src="https://img.shields.io/badge/Express.js-404D59?style=for-the-badge&logo=express&logoColor=white" alt="expressjs" />

- <h4> web development </h4>

  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white" alt="nextjs" />
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" alt="react" />
  <img src="https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white" alt="vite" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white" alt="tailwind css" />

- <h4> deployment & operations </h4>

  <img src="https://img.shields.io/badge/Ubuntu_VPS-E95420?style=for-the-badge&logo=ubuntu&logoColor=white" alt="ubuntu vps" />
  <img src="https://img.shields.io/badge/PM2-2B037A?style=for-the-badge&logo=pm2&logoColor=white" alt="pm2" />
  <img src="https://img.shields.io/badge/Netlify-00C7B7?style=for-the-badge&logo=netlify&logoColor=white" alt="netlify" />
  <img src="https://img.shields.io/badge/SSH-222222?style=for-the-badge&logo=gnubash&logoColor=white" alt="ssh" />
  <img src="https://img.shields.io/badge/DNS-111111?style=for-the-badge&logo=cloudflare&logoColor=white" alt="dns" />

- <h4> security & binary analysis </h4>

  <img src="https://img.shields.io/badge/Binary_Analysis-111111?style=for-the-badge&logo=gnometerminal&logoColor=white" alt="binary analysis" />
  <img src="https://img.shields.io/badge/PE_Analysis-2B2D42?style=for-the-badge&logo=windows&logoColor=white" alt="pe analysis" />
  <img src="https://img.shields.io/badge/YARA-4B5563?style=for-the-badge&logo=files&logoColor=white" alt="yara" />
  <img src="https://img.shields.io/badge/MSYS2_GCC-222222?style=for-the-badge&logo=c&logoColor=white" alt="msys2 gcc" />

<br /><br />

  <div align="right">
    <a href="https://discord.com/">ANYTALK</a>
  </div>
</div>

<p align="right">
  <a href="https://discord.gg/b8wQHRUjaN">
    <img src="https://img.shields.io/badge/Discord%20Server-5865F2?style=for-the-badge&logo=discord&logoColor=white" alt="Discord Server" />
  </a>
  <a href="https://crawlblog.xyz">
    <img src="https://img.shields.io/badge/Blog-111111?style=for-the-badge&logo=githubpages&logoColor=white" alt="Blog" />
  </a>
</p>

<br clear="both" />

<h2 align="center"> / FEATURED WORK / </h2>

<table>
  <tr>
    <td width="50%">
      <h3>Discord TempVoice App</h3>
      <p>
        Temporary voice channel system built with TypeScript and Discord.js.
        Handles slash commands, buttons, modals, ownership transfer, and channel controls.
      </p>
      <p>
        <b>TypeScript · Discord.js · Node.js</b>
      </p>
      <a href="https://github.com/bongmyung-ye/discord-tempvoice-app">View repository</a>
    </td>
    <td width="50%">
      <h3>Binary Analysis Lab</h3>
      <p>
        Safe reverse engineering and binary analysis lab using self-built Windows samples,
        PE triage notes, YARA rules, and Python helper scripts.
      </p>
      <p>
        <b>C · Python · YARA · PE Analysis</b>
      </p>
      <a href="https://github.com/bongmyung-ye/binary-analysis-lab">View repository</a>
    </td>
  </tr>
</table>

<h2 align="center"> / DEVELOPMENT ACTIVITY / </h2>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=bongmyung-ye&theme=github-compact&hide_border=true&area=true&custom_title=Contribution%20Graph&v=20260708" alt="GitHub activity graph" />
</p>

---

Last Edited on: 07/09/2026
